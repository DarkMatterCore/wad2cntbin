...
static const u32 g_supportedDLCs[] = {
    ...
    0x735A4700, /* "sZGx" (custom generation). */
    0x735A4800, /* "sZHx" (custom generation). */
...
bool binGenerateContentBinFromUnpackedInstallableWadPackage(os_char_t *unpacked_wad_path, os_char_t *out_path, TitleMetadata *tmd, bool skip_bins)
{
    ...
    if (skip_bins && (content_bin = os_fopen(out_path, OS_MODE_READ)))
    {
        printf("Output file \"" OS_PRINT_STR "\" already exists. Skipping...", out_path);
        success = true;
        goto out;
    }
...
bool binGenerateIndexedPackagesFromUnpackedInstallableWadPackage(os_char_t *unpacked_wad_path, os_char_t *out_path, TitleMetadata *tmd, u64 parent_tid, bool skip_bins, bool use_null_key)
{
    ...
    for(u16 i = 0; i < content_count; i++)
    {
        ...
        /* Check if this file already exists. */
        if (skip_bins && (indexed_bin = os_fopen(out_path, OS_MODE_READ)))
        {
            fclose(indexed_bin);
            fclose(cnt_fd);
            printf("Output file \"" OS_PRINT_STR "\" already exists. Skipping...", out_path);
        ...
        if (indexed_bin) fclose(indexed_bin);
